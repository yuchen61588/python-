...
const analysisOptions = [{ label: "回归预测", value: "regression" }];
...
const extractFeatureGroups = () => {
  ...
  allColumns.forEach((column) => {
    ...
    // 按城市_特征类型格式进行分类
      ...
      column.includes("_temp_") ||
      column.toLowerCase().includes("_temperature")
    ) {
      defaultFeatures.temperature.push(column);
      ...
      column.includes("_pressure") ||
      column.includes("_cloud_cover") ||
      column.includes("_wind_") ||
      column.includes("_global_radiation") ||
    ...
    allColumns.forEach((column) => {
      if (
        (column.toLowerCase().includes("temp") ||
          column.toLowerCase().includes("temperature")) &&
    ...
    allColumns.forEach((column) => {
      if (
        (column.toLowerCase().includes("humidity") ||
          column.toLowerCase().includes("humid")) &&
  ...
  if (defaultFeatures.precipitation.length === 0) {
    allColumns.forEach((column) => {
      if (
        (column.toLowerCase().includes("precip") ||
          column.toLowerCase().includes("rain")) &&
...
const initFeatures = () => {
  ...
  // 获取所有可用的数值列（排除日期）
  const availableColumns = dataStore.currentData
    ? Object.keys(dataStore.currentData[0])
    : [];

  // 尝试通过检查首行数据确定数值列
  const numericColumns = availableColumns.filter((column) => {
    if (defaultFeatures.date.includes(column) && !column.includes("_numeric"))
      return false;
  ...
  if (effectiveNumericColumns.length < 3 && defaultFeatures.bbq.length > 0) {
    const bbqColumns = defaultFeatures.bbq.filter((col) =>
      col.includes("_numeric")
    );
  ...
  if (analysisMethod.value === "clustering") {
    // 优先使用BBQ数值版本特征
    const bbqNumericCols = effectiveNumericColumns.filter(
      (col) => col.includes("BBQ") && col.includes("_numeric")
    );
  ...
  // 设置默认回归特征
    ...
    if (bbqNumericCols.length > 0) {
      ...
      // 添加剩余的BBQ特征
      const otherBbqCols = bbqNumericCols
        .filter((col) => col !== analysisParams.targetFeature)
        .slice(0, 2);
      ...
      // 添加时间特征
      const timeFeatures = effectiveNumericColumns
        .filter(
          (col) =>
      ...
      // 添加随机特征
      const randomFeatures = effectiveNumericColumns
        .filter(
          (col) => col.includes("random_feature") && !predictors.includes(col)
        )
        .slice(0, 2);
      ...
      if (predictors.length < 5) {
        const otherFeatures = effectiveNumericColumns
          .filter(
            (col) =>
              !predictors.includes(col) && col !== analysisParams.targetFeature
          )
...
const fetchDataFiles = async () => {
  try {
    ...
    // 查找天气数据文件
    const weatherFile = dataFiles.value.find(
      (file) =>
    ...
    if (weatherFile) {
      ...
      if (dataStore.currentData && dataStore.currentData.length > 0) {
        ...
        noDataMessage.value =
          "找到天气数据文件，但没有加载数据内容，请先在数据管理页面预览数据";
      }
    } else {
      noDataMessage.value =
        "找不到天气数据文件，请先上传weather_prediction_dataset.csv";
    }
  } catch (error) {
...
const numericFeatures = computed(() => {
  return availableFeatures.value.filter(
...
const runAnalysisTask = async () => {
  ...
  try {
    ...
    if (validFeatures.length === 0) {
      ...
      const bbqCols = availableColumns.filter(
        (col) =>
          col.includes("BBQ_weather") ||
          (col.includes("BBQ") && col.includes("_numeric"))
      );

      if (bbqCols.length > 0) {
        // 优先使用BBQ数值版本
        const bbqNumericCols = bbqCols.filter((col) =>
          col.includes("_numeric")
        );
        if (bbqNumericCols.length > 0) {
      ...
      // 尝试使用随机特征
        ...
        // 尝试使用非日期列
          ...
          const nonDateCols = availableColumns
            .filter(
              (col) =>
                col !== "DATE" &&
                col !== "MONTH" &&
      ...
      // 最终验证
      const finalValidFeatures = selectedFeatures.filter((f) =>
        availableColumns.includes(f)
      );
    ...
    if (analysisMethod.value === "regression") {
      if (!analysisParams.targetFeature) {
        ...
        const randomCols = availableColumns.filter(
          (col) =>
            col.includes("random_feature") && !selectedFeatures.includes(col)
        );
        ...
        if (bbqCols.length > 0) {
          ...
          console.log(
            "自动选择BBQ列作为目标特征:",
            analysisParams.targetFeature
          );
          ElMessage.info(
            `自动选择 "${analysisParams.targetFeature}" 作为目标特征`
          );
      ...
      if (!availableColumns.includes(analysisParams.targetFeature)) {
        ...
        if (newTarget) {
          console.log(
            `目标特征 "${analysisParams.targetFeature}" 不存在，使用 "${newTarget}" 替代`
          );
          ElMessage.warning(
            `目标特征 "${analysisParams.targetFeature}" 不存在，使用 "${newTarget}" 替代`
          );
          ...
          ElMessage.error(
            `目标特征 "${analysisParams.targetFeature}" 在数据中不存在，且无法找到替代特征`
          );
          console.error(
            `目标特征 "${analysisParams.targetFeature}" 在数据中不存在，且无法找到替代特征`
          );
      ...
      selectedFeatures = selectedFeatures.filter(
        (f) => f !== analysisParams.targetFeature
      );
      ...
      if (selectedFeatures.length === 0) {
        ...
        if (bbqCols.length > 0) {
          ...
          const newFeatures = availableColumns
            .filter(
              (col) =>
                col !== "DATE" &&
                col !== "MONTH" &&
    ...
    if (analysisMethod.value === "clustering") {
      ...
      if (analysisParams.regressionAlgorithm === "linear") {
        ...
        parameters.polynomial_degree =
          analysisParams.linearOptions.polynomialDegree;
      }
    }
...
const generateMockResults = () => {
  ...
  // 使用已选择的特征或自动选择特征
  const features =
    analysisParams.selectedFeatures.length > 0
      ? analysisParams.selectedFeatures
      : Object.keys(data[0])
          .filter(
            (key) =>
              key !== "DATE" && key !== "MONTH" && !key.includes("BBQ_weather")
          )
          .slice(0, 5);
  ...
  switch (analysisMethod.value) {
    case "clustering":
      ...
      // 计算每个聚类的数量
      const clusterCounts = Array(clusterCount)
        .fill(0)
        .map((_, i) => clusterLabels.filter((label) => label === i).length);

      // 生成聚类中心（半随机，让BBQ列的值更有意义）
      const clusterCenters = Array(clusterCount)
        .fill(0)
        .map((_, cluster) => {
          const center = {};
...
const renderAnalysisChart = () => {
  ...
  switch (analysisResult.value.type) {
    case "clustering":
      ...
      const clusterCounts =
        ...
        (result.clusters
          ? Array.from(new Set(result.clusters)).map(
              (cluster) => result.clusters.filter((c) => c === cluster).length
            )
          : []);
    ...
    case "regression":
      ...
      const rangeBuffer = Math.max(
        1,
        (Math.max(maxActual, maxPredicted) -
          Math.min(minActual, minPredicted)) *
          0.2
      );
      const axisMin = Math.floor(
        Math.min(minActual, minPredicted) - rangeBuffer
      );
      const axisMax = Math.ceil(
        Math.max(maxActual, maxPredicted) + rangeBuffer
      );

      option.title.subtext = `目标特征: ${
        result.target || analysisParams.targetFeature
      }`;
      ...
      option.tooltip = {
        ...
        formatter: function (params) {
          if (params.seriesName === "预测 vs 实际") {
            ...
            const errorPercent =
              actual !== 0
                ? ((error / Math.abs(actual)) * 100).toFixed(1)
                : "∞";

            return (
              `实际值: ${actual.toFixed(2)}<br/>` +
      ...
      if (isRandomForest) {
        ...
        option.series.push({
          ...
          type: "line",
          data: [
            [minActual, minActual * slope + intercept],
            [maxActual, maxActual * slope + intercept],
          ],
          lineStyle: {
            type: "dashed",
            color: "#ee6666",
  ...
  // 如果是随机森林模型，添加特征重要性图表
  if (
    analysisResult.value.type === "regression" &&
    analysisResult.value.algorithm === "random_forest"
  ) {
...
const renderResidualHistogram = () => {
  ...
  if (
    ...
    !analysisResult.value.result.predictions
  ) {
    return;
  }
...
const renderFeatureImportanceChart = () => {
  ...
  if (
    !analysisResult.value ||
    !analysisResult.value.result ||
    !analysisResult.value.result.coefficients
  ) {
    return;
  }
  ...
  // 准备数据
  const featureNames =
    analysisResult.value.result.feature_names ||
    analysisParams.selectedFeatures;
...
const getChartTitle = () => {
  ...
  switch (result.type) {
    case "clustering":
      return `聚类分析结果 (${getAlgorithmLabel(
        result.algorithm,
        "clustering"
      )})`;
    case "regression":
      return `回归分析结果 (${getAlgorithmLabel(
        result.algorithm,
        "regression"
      )})`;
    default:
...
const setDefaultFeatures = () => {
  ...
  // 优先使用BBQ数值特征
  const bbqNumericCols = availableColumns.filter(
    (col) => col.includes("BBQ") && col.includes("_numeric")
  );
  ...
  // 如果没有BBQ数值特征，使用随机特征
  const randomFeatures = availableColumns.filter((col) =>
    col.includes("random_feature")
  );
  if (randomFeatures.length > 0) {
    if (analysisMethod.value === "clustering") {
  ...
  <div class="data-analysis">
    <el-card v-loading="loading">
      ...
      <el-empty v-if="noDataMessage" :description="noDataMessage">
        <el-button type="primary" @click="$router.push('/data-management')"
          >去上传数据</el-button
        >
      </el-empty>
      ...
      <div v-else>
        <el-form label-width="120px">
          ...
          <el-form-item label="分析特征">
            <div class="selected-features">
              <el-tag
                ...
                @close="
                  analysisParams.selectedFeatures =
                    analysisParams.selectedFeatures.filter((f) => f !== feature)
                "
              >
                {{ feature }}
              </el-tag>
              ...
              <div
                v-if="analysisParams.selectedFeatures.length === 0"
            ...
            <div class="feature-actions">
              <el-button type="primary" size="small" @click="setDefaultFeatures"
                >重置默认特征</el-button
              >
              <el-button
                type="primary"
                size="small"
                @click="showFeatureSelector = true"
                >添加/修改特征</el-button
              >
            </div>
          </el-form-item>
          ...
          <template v-if="analysisMethod === 'clustering'">
            <el-form-item label="聚类算法">
              <el-select
                v-model="analysisParams.clusteringAlgorithm"
                style="width: 100%"
              >
                <el-option
            ...
            <el-form-item label="聚类数量">
              <el-slider
                v-model="analysisParams.clusterCount"
                :min="2"
                :max="6"
              />
            </el-form-item>
          ...
          <template v-if="analysisMethod === 'regression'">
            <el-form-item label="目标特征">
              <el-select
                v-model="analysisParams.targetFeature"
                style="width: 100%"
              >
                <el-option
            ...
            <el-form-item label="回归算法">
              <el-select
                v-model="analysisParams.regressionAlgorithm"
                style="width: 100%"
              >
                <el-option
            ...
            <template v-if="analysisParams.regressionAlgorithm === 'linear'">
              ...
              <el-form-item label="线性回归类型">
                <el-select
                  v-model="analysisParams.linearOptions.linearType"
                  style="width: 100%"
                >
                  <el-option
              ...
              <el-form-item
                v-if="analysisParams.linearOptions.linearType !== 'standard'"
                label="正则化强度"
              >
                <el-slider
              ...
              <el-form-item label="多项式特征">
                <el-switch
                  v-model="analysisParams.linearOptions.usePolynomial"
                />
                <span class="option-hint"
                  >使用多项式特征可能提高模型对非线性关系的拟合能力</span
                >
              </el-form-item>

              <el-form-item
                v-if="analysisParams.linearOptions.usePolynomial"
                label="多项式阶数"
              >
                <el-slider
          ...
          <el-form-item>
            <el-button type="primary" @click="runAnalysisTask"
              >运行分析</el-button
            >
          </el-form-item>
        </el-form>
        ...
        <!-- 特征选择器对话框 -->
        <el-dialog v-model="showFeatureSelector" title="特征选择" width="60%">
          ...
          <template #footer>
            <span class="dialog-footer">
              ...
              <el-button type="primary" @click="showFeatureSelector = false"
                >确认</el-button
              >
            </span>
          </template>
            ...
            <div class="result-metrics">
              <el-descriptions :column="2" border>
                ...
                <el-descriptions-item label="聚类数">
                  {{
                    analysisResult.result.clusterCounts
                      ? analysisResult.result.clusterCounts.length
                      : 0
                  }}
                </el-descriptions-item>
              </el-descriptions>
              ...
              <el-table
                v-if="analysisResult.result.clusterCounts"
                :data="
                  analysisResult.result.clusterCounts.map((count, index) => ({
                    cluster: `聚类 ${index + 1}`,
                    count,
                  }))
                "
            ...
            <div class="result-metrics">
              <el-descriptions :column="3" border>
                ...
                <el-descriptions-item label="决定系数 R²">
                  {{
                    analysisResult.result.metrics?.r2.toFixed(4) ||
                    analysisResult.result.r2_score?.toFixed(4) ||
                    "0.0000"
                  }}
                </el-descriptions-item>
                ...
                <el-descriptions-item label="特征重要性">
                  <div v-if="analysisResult.result.coefficients">
                    <div
                      v-for="(coef, index) in analysisResult.result
                        .coefficients"
                      :key="index"
                      :style="{ color: getCoefColor(coef) }"
                    >
                      {{
                        analysisResult.result.feature_names?.[index] ||
                        analysisParams.selectedFeatures[index]
                      }}:
              ...
              <div
                v-if="
                  analysisResult.algorithm === 'linear' &&
                  analysisResult.result.extra_info
                "
                class="advanced-info"
              >
                <el-alert type="info" :closable="false" show-icon>
                  ...
                  <template #default>
                    <div>
                      <p>
                        ...
                        {{
                          getLinearTypeLabel(
                            analysisResult.result.extra_info.linear_type
                          )
                        }}
                      </p>
                      <p>
                        <strong>特征缩放:</strong>
                        {{
                          analysisResult.result.extra_info.scaled
                            ? "已应用"
                            : "未应用"
                        }}
                      </p>
                      <p v-if="analysisResult.result.extra_info.polynomial">
                        ...
                        已应用 (阶数:
                        {{
                          analysisResult.result.extra_info.polynomial.degree
                        }})
                      </p>
                    </div>
                  </template>
              ...
              <div
                v-if="analysisResult.algorithm === 'random_forest'"
                class="forest-info"
              >
                <el-alert type="info" :closable="false" show-icon>
