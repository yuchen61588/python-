...
const fetchDataFiles = async () => {
  try {
    loading.value = true; //靠在状态界面
    const response = await getDataFiles();
    dataFiles.value = response.data; //存储已上传的文件列表
  } catch (error) {
...
const previewFile = async (fileId) => {
  try {
    loading.value = true;
    const response = await getDataFilePreview(fileId); //预览数据，文件头
    //返回数据 id name 预览列表 列数 行数
    if (response.data) {
...
const processData = (data) => {
  ...
  if (data.columns && data.data) {
    ...
    columns.value = data.columns.map((col) => ({
      prop: col,
      label: col,
    })); //定义标题列 data.columns 中的每个列名转换为一个对象。
...
const handleExport = async () => {
  ...
  loading.value = true;
  try {
    const response = await exportData(currentFile.value.id, "csv"); //文件导出后端请求
    const url = window.URL.createObjectURL(new Blob([response.data]));
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute(
      "download",
      `${currentFile.value.name}_export_${new Date().getTime()}.csv`
    );
        ...
        <template #header>
          <div class="card-header">
            ...
            <div class="step-buttons" v-if="tableData.length > 0">
              <el-button type="primary" @click="startCleaning"
                >开始数据清洗</el-button
              >
            </div>
          </div>
            ...
            <el-table :data="dataFiles" border style="width: 100%">
              ...
              <el-table-column label="操作" width="180">
                <template #default="scope">
                  <el-button size="small" @click="selectFile(scope.row)"
                    >查看</el-button
                  >
                </template>
              </el-table-column>
